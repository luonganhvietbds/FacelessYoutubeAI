...
import { Languages, Moon, RotateCcw, Shield, LogIn, LogOut, User, Loader2, Key } from 'lucide-react';
...
export function Header() {
    ...
    const { hasActiveKeys, keys } = useApiKeys();
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <Button
                                ...
                                className={`text-zinc-400 hover:bg-zinc-800 ${hasActiveKeys ? 'hover:text-emerald-400' : 'hover:text-yellow-400'
                                    }`}
                                title={hasActiveKeys ? `${activeKeyCount} API keys active` : 'Add API keys'}
                            >
                                <Key className="w-4 h-4 mr-2" />
                                <span className="hidden sm:inline">
                                    {hasActiveKeys ? `${activeKeyCount} Keys` : 'Add Keys'}
                                </span>
                                {hasActiveKeys && (
